...
nDiasFerias = 30                # Número de dias de férias por trabalhadaor
...
def criterio2(horario, fds, nDiasTrabalhoFDS, feriados):
    trabalhadores, dias, turnos = horario.shape

    # Cria máscaras para os finais de semana e feriados
    mask_fds = np.expand_dims(fds, axis=2)  # (nTrabs, nDias, 1)
    mask_fds = np.broadcast_to(mask_fds, (trabalhadores, dias, turnos))

    feriados_mask = np.isin(np.arange(dias), feriados).astype(bool)  # (nDias,)
    feriados_mask = feriados_mask[None, :, None]  # (1, nDias, 1)
    feriados_mask = np.broadcast_to(feriados_mask, (trabalhadores, dias, turnos))

    # Aplica as máscaras
    mask = mask_fds | feriados_mask  # FDS ou feriados
    trabalhos_em_fds_ou_feriado = horario * mask
    total_dias_fds_feriado = np.sum(trabalhos_em_fds_ou_feriado, axis=(1, 2))

    # Calcula o excedente de dias trabalhados
    excedente = np.maximum(total_dias_fds_feriado - nDiasTrabalhoFDS, 0)
    return excedente
...
def criterio3(horario, nMinTrabs):
    ...
    dias_com_menos_trabs = np.sum(trabalhadores_por_dia < nMinTrabs, axis=1)
    return np.sum(dias_com_menos_trabs)
...
def criterio5(horario, Prefs):
    t = horario[:, :-1, 1]  # Tarde hoje
    m = horario[:, 1:, 0]   # Manhã amanhã
    violacoes = np.sum(t & m, axis=1)

    mask_pref = np.array([(0 in p or 1 in p) for p in Prefs])
    violacoes_pref = violacoes * mask_pref

    return violacoes_pref
